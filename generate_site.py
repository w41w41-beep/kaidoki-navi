...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 最初のカテゴリーのサブカテゴリーをデフォルトで表示する（オプション）
    // if (Object.keys(PRODUCT_CATEGORIES).length > 0) {
    //     renderSubCategories(Object.keys(PRODUCT_CATEGORIES)[0], basePath);
    // }
    
    // 特別カテゴリー（タグ、最安値、セール）のリンクも追加
    const specialLinks = [
        ["タグから探す", `${basePath}tags/index.html`],
        ["最安値", `${basePath}category/最安値/index.html`],
        ["期間限定セール", `${basePath}category/期間限定セール/index.html`]
    ];

    // 既存のメインリンクが動的に生成されている場合は、この処理は不要かもしれません
    // が、念のため、特別なリンクが追加されていないか確認してください。
});
